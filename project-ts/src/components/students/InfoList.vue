...
<script>
import { getStudentList, removeStudent, addStudentInfo, updateStudent } from '@/api/api.ts'
export default {
    data() {
        return {
            tableData: [],
    ...
    methods: {
        ...
        editData(row) {
            ...
            this.form = { ...row }; //写法类似拷贝行数据的副本，不然直接获取row的话，会是当前行的实时数据
        },
        addStudent() {
            this.form = {
                name: '',
